...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var summed = A3($List.foldl,F2(function (x,y) {    return x + y;}),0,_U.list([1,2,3,4,5]));
   var keys = {x: -1,y: 1};
   var dt = 100;
   var inputSignal = function () {
      var delta = $Time.fps(30);
      var tuples = A3($Signal.map2,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),delta,$Keyboard.arrows);
      return A2($Signal.sampleOn,delta,tuples);
   }();
   var Keys = F2(function (a,b) {    return {x: a,y: b};});
   var updateShooting = F2(function (isShooting,ship) {    return _U.update(ship,{shooting: isShooting});});
   var updateVelocity = F2(function (newVelocity,ship) {    return _U.update(ship,{velocity: newVelocity});});
   var applyPhysics = F2(function (dt,ship) {    return _U.update(ship,{position: ship.position + ship.velocity * dt});});
   var update = F3(function (dt,keys,ship) {
      var isShooting = _U.cmp(keys.y,0) > 0;
      var newVel = $Basics.toFloat(keys.x);
      return A2(updateVelocity,newVel,A2(updateShooting,isShooting,A2(applyPhysics,dt,ship)));
   });
   var initShip = {position: 0,velocity: 0,shooting: false};
   var main = $Graphics$Element.show(A3(update,dt,keys,initShip));
   var Ship = F3(function (a,b,c) {    return {position: a,velocity: b,shooting: c};});
